...
Basic information about the files can be displayed with \texttt{print()}, or just by entering the name of a DVH object at the prompt -- here used with built-in DVHs from three patients with radiotherapy after breast cancer surgery, each with seven heart structures.\footnote{Data courtesy of Department of Radiation Oncology (Prof. Dr. Schmidberger), University Medical Center Mainz, Germany.}
...
The authors thank Marcus Stockinger for ideas on checking quality assurance constraints, and Sandra B\"{u}hrdel for example DVH files exported from Cadplan.
