...
To import DVH data into \textsf{R}, it should be exported as a plain text file from Varian Eclipse$^{\mathrm{TM}}$ (Versions 10--13), from CadPlan$^{\mathrm{TM}}$, or from Oncentra MasterPlan$^{\mathrm{TM}}$ (version 4.3). Cumulative and differential DVHs are supported, as are sum plans. The measurement unit for absolute dose can be Gy or cGy, the measurement unit for volume has to be cm$^{3}$. DVH text files can be read with \texttt{readDVH()}.
...
\item \texttt{"DEUD"}: The generalized equivalent uniform dose (gEUD, \citeNP{Niemierko1999,Wu2002}).
\item \texttt{"DEUD2"}: The gEUD in 2Gy fractions biologically equivalent dose.
\item \texttt{"DNTCP"}: The normal tissue complication probability (NTCP) according to the probit model \cite{Lyman1985}, the logit model \cite{Niemierko1999}, or the Poisson model \cite{Kallman1992}. For DVH reduction, gEUD is used -- which makes the probit model equivalent to the Lyman model with Kutcher-Burman DVH reduction scheme \cite{Rancati2004,Kutcher1991}.
\item \texttt{"DTCP"}: The tumor control probability (TCP) according to the probit model, the logit model, or Poisson model.
\end{itemize}
