...
Users familiar with \textsf{R} can use the \texttt{DVHmetrics} package from the \textsf{R} command line. This facilitates statistical post-processing of results with the full capabilities of \textsf{R}. To install \texttt{DVHmetrics}, you need a current version of \textsf{R} and be online. Preferably, a free development environment like \textsf{R}Studio \cite{Allaire2011} or Architect \cite{OpenAnalytics2013} should be used.

<<cIntro1, eval=FALSE>>=
# install the package from the CRAN online repository
...
For users who are unfamiliar with \textsf{R}, \texttt{DVHmetrics} includes a \texttt{shiny}-based \cite{RStudioShiny2014} web application running locally that can be employed without the need to use \textsf{R} syntax.\footnote{This app is also available online in a public install: \url{http://dwoll.shinyapps.io/DVHshiny/}} This web application can also be deployed in an intranet from local IT services.\footnote{To install separately, use function \texttt{install\_github("dwoll/DVHshiny)} from package \texttt{devtools} \cite{Wickham2014b}.} After installing \texttt{DVHmetrics}, you can start the web application from the command line in an external web browser (fig.\ \ref{fig:appFileOpen}, \ref{fig:appMetrics}, \ref{fig:appShowDVH}, \ref{fig:appCheckConstr}, \ref{fig:appShowConstr}):

<<cWebApp1, eval=FALSE>>=
# opens web browser with DVHmetrics web application
shine()
@
...
To import DVH data into \textsf{R}, it should be exported as a text file from Varian Eclipse (Aria Version 10 or 11) or from Cadplan. Cumulative and differential DVHs are supported, as are sum plans. The measurement unit for absolute dose can be Gy or cGy, the measurement unit for volume has to be cm$^{3}$. DVH text files can be read with \texttt{readDVH()} (fig.\ \ref{fig:appFileOpen} shows the web application).

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{appFileOpen}
\caption{Importing DVH text files into the \texttt{DVHmetrics} web application}
...
In the following examples, we use object \texttt{dataMZ} that is built into the \texttt{DVHmetrics} package. \texttt{dataMZ} was the result from reading 3 Eclipse DVH files, each with 7 structures -- as demonstrated in section \ref{sec:readData}  (fig.\ \ref{fig:appMetrics} shows the web application).

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{appMetrics}
\caption{Calculate DVH metrics in the \texttt{DVHmetrics} web application}
...
Calculate metrics \texttt{D10\%} and \texttt{V5Gy} for structures containing \texttt{AMYOC} or \texttt{VALVE} for patient IDs containing \texttt{23} in \texttt{dataMZ}, and sort result by metric and observed value.
...
The calculated DVH metrics can be saved to tab-delimited text files with \texttt{saveMetric()}. These files are easy to import, e.\,g., into spreadsheets like Excel or into other statistics programs. Assume object \texttt{met} has been calculated before as demonstrated in section \ref{sec:getMetric}.
...
Cumulative as well as differential DVH diagrams can be generated with \texttt{showDVH()} (fig.\ \ref{fig:appShowDVH} shows the web application). If you are using \textsf{R}Studio or Architect, all produced diagrams are accessible in the plots tab by clicking on the left and right arrows. Depending on the option \texttt{byPat}, each DVH diagram either shows 1 patient with multiple structures (\texttt{byPat=TRUE}) or 1 structure with multiple patients (\texttt{byPat=FALSE}).

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{appShowDVH}
\caption{Visualize DVHs in the \texttt{DVHmetrics} web application}
