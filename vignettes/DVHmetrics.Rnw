...
\item \texttt{"DNTCP"}: The normal tissue complication probability (NTCP) according to the Lyman \citeyear{Lyman1985} probit model, the Niemierko \citeyear{Niemierko1999} logit model, or the K\"{a}llman \citeyear{Kallman1992} Poisson model. This can be based on EQD$_{2}$ values if information on the fractionation is provided as well.
\item \texttt{"DTCP"}: The tumor control probability (TCP) according to the Lyman probit model, the Niemierko logit model, or the K\"{a}llman Poisson model. This can be based on EQD$_{2}$ values if information on the fractionation is provided as well.
\end{itemize}
