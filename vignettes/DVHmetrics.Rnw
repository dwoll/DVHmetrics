...
pdfauthor={Daniel Wollschlaeger, Heiko Karle},%
...
\texttt{DVHmetrics} is an add-on package for the free statistical environment \textsf{R}\footnote{Project homepage: \url{http://r-project.org/} -- free short introductions to \textsf{R} can be found at\\ \url{http://www.splusbook.com/RIntro/RCourse.pdf} and \url{http://www.statmethods.net/}.} with applications in radiation oncology. It provides functionality to read dose-volume-histogram (DVH) text files, calculate DVH metrics, and to plot DVHs. In addition, it checks and visualizes quality assurance constraints for the DVH.\footnote{For a solution that also reads files in DICOM-RT format and has a broader scope in supporting radiation oncology, see the \texttt{RadOnc} package for \textsf{R}: \url{http://CRAN.R-project.org/package=RadOnc}.}
...
\texttt{DVHmetrics} provides two interfaces geared towards users with different levels of familiarity with \textsf{R}.
...
For users who are unfamiliar with \textsf{R}, the web application \texttt{DVHshiny}\footnote{\url{http://dwoll.shinyapps.io/DVHshiny/} -- The source code can be found at \url{https://github.com/dwoll/DVHshiny}.} provides interactive access to the functionality of \texttt{DVHmetrics} without the need to use \textsf{R} syntax (fig.\ \ref{fig:DVHshiny01}, \ref{fig:DVHshiny02}). This web application can also be installed locally, or can be deployed in an intranet from local IT services.\footnote{To install, use function \texttt{install\_github("dwoll/DVHshiny)} from package \texttt{devtools}.}

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{DVHshiny01}
\label{fig:DVHshiny01}
\caption{Calculating DVH metrics in \texttt{DVHshiny}}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=10cm]{DVHshiny02}
\label{fig:DVHshiny02}
\caption{Visualizing DVH constraints in \texttt{DVHshiny}}
\end{figure}

%%%%---------------------------------------------------------------------------
%%%%---------------------------------------------------------------------------
\section{\textsf{R} interace}
\label{sec:Rinter}
%%%%---------------------------------------------------------------------------
%%%%---------------------------------------------------------------------------

Users familiar with \textsf{R} can use the \texttt{DVHmetrics} package from the \textsf{R} command line as demonstrated in this document. This facilitates statistical post-processing of results with the full capabilities of \textsf{R}. To install \texttt{DVHmetrics}, you need a current version of \textsf{R} (and, preferably, the free development environment \textsf{R}Studio\footnote{\url{http://www.rstudio.com/ide/download/desktop}}) and be online:

<<cIntro1, eval=FALSE>>=
install.packages("DVHmetrics")
@

After installing \texttt{DVHmetrics}, you should be able to run:

<<cIntro2>>=
## load DVHmetrics package - required for all following tasks
library(DVHmetrics, verbose=FALSE)

## run an analysis on built-in data
getMetric(dataMZ, metric="DMEAN", structure="HEART")
@

Function \texttt{getMetric()} used above is explained in section \ref{sec:getMetric}.
...
res <- readDVH(type="Eclipse")           # opens file picker
@
...
\end{itemize}
\item A \emph{free} DVH metric is a character string which has 3 mandatory elements and one optional element in the following order:
\begin{itemize}
...
\item Optional 4$^{\mathrm{th}}$ element \texttt{\_$\langle$measurement unit$\rangle$}: The measurement unit of the output value. Possible units are as for the 3$^{\mathrm{rd}}$ element. If missing, dose is reported as absolute dose in the measurement unit used in the DVH. Volume is then reported as relative volume in \%.
\end{itemize}
...
\item Option \texttt{patID}: Which patient IDs should be analyzed. IDs can be regular expressions matched against those found in the DVH files. Also supply \texttt{fixed=TRUE} for exact matching.

\item Option \texttt{structure}: Which structure should be analyzed. Structure names can be regular expressions matched against those found in the DVH files. Also supply \texttt{fixed=TRUE} for exact matching.
...
\item \texttt{"structure"}: structure for which metric is calculated
...
\begin{table}[!htbp]
\caption{Examples for possible DVH metrics}
\label{tab:metrics}
\centering
\begin{tabular}{lllll}
metric                & reference       & unit ref & result          & unit result \\\hline
...
\texttt{"DMAX"}       & ---             & ---      & absolute dose   & as in DVH \\
\end{tabular}
\end{table}

If volume or dose values outside the range of possible values for a structure are requested, metrics cannot be calculated, and the result will be \texttt{NA} with a warning.

In the following examples, we use object \texttt{dataMZ} that is built into the \texttt{DVHmetrics} package. \texttt{dataMZ} was the result from reading 3 Eclipse DVH files, each with 7 structures -- as demonstrated in section \ref{sec:readData}.
...
Calculate metric \texttt{D5cc} just for structure \texttt{HEART} for all patients in \texttt{dataMZ}, and sort result by the value of the metric.

<<cMetrics3>>=
getMetric(dataMZ, metric="D5cc", structure="HEART", sortBy="observed")
@

Calculate metrics \texttt{D10\%} and \texttt{V1Gy} for structures containing \texttt{AMYOC} or \texttt{VALVE} for patient IDs containing \texttt{23} in \texttt{dataMZ} and sort result by metric and observed value.

<<cMetrics4>>=
getMetric(dataMZ, metric=c("D10%", "V1Gy"),
          structure=c("AMYO", "VALVE"),
          patID="23",
          sortBy=c("metric", "observed"))
@

Calculate metrics \texttt{DMEAN} and \texttt{D5cc} for structure \texttt{HEART} for all patients in \texttt{dataMZ}, sort by value of the metric, and split the output such that one table is generated for each metric.
...
The calculated DVH metrics can be saved to tab-delimited text files with \texttt{saveMetric()}. These files are easy to import, e.\,g., in spreadsheets like Excel or other statistics programs. Assume object \texttt{met} has been calculated before as demonstrated in section \ref{sec:getMetric}.

If \texttt{met} is not split into different tables, the following command saves \texttt{met} to the file \texttt{metrics.txt}. If \texttt{met} is divided into multiple tables, this saves \texttt{met} into different files that all have the name pattern \texttt{metrics\_NAME.txt}, where \texttt{NAME} stands, e.\,g., for the names of different structures.
...
%%%%---------------------------------------------------------------------------
%%%%---------------------------------------------------------------------------
\section{Cumulative DVH diagrams}
\label{sec:plot}
%%%%---------------------------------------------------------------------------
%%%%---------------------------------------------------------------------------
...
%%%%---------------------------------------------------------------------------
%%%%---------------------------------------------------------------------------
\subsection{Plot cumulative DVH diagrams}
\label{sec:showDVH}
%%%%---------------------------------------------------------------------------
%%%%---------------------------------------------------------------------------

Cumulative as well as differential DVH diagrams can be generated with \texttt{showDVH()}. If you are using \textsf{R}Studio, all produced diagrams are accessible in the plots tab by clicking on the left and right arrows. Depending on the option \texttt{byPat}, each DVH diagram either shows 1 patient with multiple structures (\texttt{byPat=TRUE}) or 1 structure with multiple patients (\texttt{byPat=FALSE}).
...
showDVH(dataMZ, byPat=FALSE, patID="P234")
@
...
Option \texttt{thresh} allows to restrict the range of the $x$-axis such that only volumes larger than \texttt{thresh} appear. Use option \texttt{show=FALSE} to prevent the diagrams from being shown if you just need the returned object (here: \texttt{dvhPlot}) to later save the diagrams to file.
...
As an example, we use the DVHs that are built into the \texttt{DVHmetrics} package.
...
The authors thank Marcus Stockinger for ideas on checking quality assurance constraints.
